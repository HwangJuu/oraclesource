--PL/SQL
-- scott로 연결
--SQL를 프로그래밍 한다.SQL인데 프로그램이 들어있다.
...
--묵시적 커서(커서를 선언하지 않아도 알아서 커서가 돌아감) : 커서 선언 없이 사용
--SELECT ~ INTO / DML(update/delete/insert)
--SQL%ROWCOUNT : 무시적 커서 안에 추출된 행이 있으면 행의 수 출력
--SQL%FOUND : 묵시적 커서 안에 추출된 행이 있으면 TURE, 없으면 FALSE
--SQL%ISOPEN : 자동으로 SQL문을 실행 한 후 CLOSE 되기 때문에 항상 FALSE
--SQL%NOTFOUND : 커서안에 추출된 행이 있으면 TRUE, 없으면 FALSE

BEGIN
     UPDATE dept_temp SET dname = 'DATABASE' WHERE deptno = 60;
     
     dbms_output.put_line('갱신된 행의 수 : ' || SQL%FOWCOUNT);
...
-- 저장 서브 프로그램(이름지정, 저장, 저장할 떄 한번 컴파일, 공유해서 사용 가능
-- 다른 응용프로그램에서 호출 가능)
-- 1. 저장 프로시저 : sql 문에서는 사용 불가
-- 2. 저장 함수 : sql 문에서 사용 가능
-- 3. 트리거 : 특정상황이 발생 할때 자동으로 연달아 수행 할 기능을 구현하는데 사용
-- 4. 패키지 : 저장 서브 프로그램을 그룹화 할 때 사용
...
-- emp_trg insert (주말에 사원 정보 추가시 에러), update, delete
...
-- error number argument to raise_application_error of -30001 is out of range
UPDATE emp_trg
SET sal=3500
WHERE empno=7369;

-- error number argument to raise_application_error of -30002 is out of range
...
AFTER
INSERT OR UPDATE OR DELETE ON emp_trg
FOR EACH ROW --행별로 각각 실행하겠다. 영향받는 모든 행별 실행
BEGIN
     IF INSERTING THEN
...
--날짜 주말로 변경시 : error number argument to raise_application_error of -30001 is out of range
UPDATE emp_trg
SET sal=3600
WHERE empno=7369;
