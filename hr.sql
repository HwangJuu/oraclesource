...
-- 위 세개의 열은 별칭을 붙여서 조회
-- 조회 할 때 부서번호를 기준으로 내림차순으로 정렬하되 부서 번호가 같다면
...
--[문제 4] 사원번호가 176인 사원의 last_name과 deptno 조회
SELECT
    last_name,
    department_id
FROM
...
--[문제 5] 연봉이 12,000 이상 되는 직원들의 last_name 과 salary조회
SELECT
    last_name,
    salary
FROM
...
--[문제 6] 연봉이 5000 ~ 12000 범위 사이가 아닌 사원들 조회
SELECT
    last_name,
    salary
FROM
...
--[문제 7] 20번 혹은 50번 부서에서 근무하는 모든 사원들의 LAST_NAME 및 DEPARTMENT_ID 조회
...
--[문제8] 커미션을 받는 모든 사원들의 LAST_NAME,SALARY,COMMISSIONT_PCT를 조회한다
...
--[문제9] 연봉이 2500,3500,7000이 아니며, 직업이 SA_REP, ST_CLERK 인 사원 조회
--전체 정보 조회
SELECT
    *
FROM
...
--[문제] commision_pct가 널이 아닌 사원들 중에서 commission = salart*commion_pct를 구하여,
...
--출력(employess, departments, locations 테이블 조인)
SELECT
    employee_id,
    last_name,
    d.department_id,
...
--department_name, location_id,sal_avg, cnt출력
--(employess, departments 조인)
SELECT
    department_name,
    location_id,
...
--departmen_id, last_name, salary, hire_date 출력
--employees 셀프 조인
SELECT DISTINCT
    e2.department_id,
    e2.last_name,
